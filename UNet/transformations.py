...
def normalize_01(inp: np.ndarray):
    ...
    print("normalize 01 shape: ", inp.shape)
    # H, W, C = inp.shape
    # inp.reshape((-1, H, W))
...
def normalize(  inp: np.ndarray,
    ...
    """ Normalize based on mean and stadard deviation """
...
def center_crop_to_size(x: np.ndarray,
    ...
    """
    Center crops a given array x to the sized passed into the function
...
def random_flip(
    ...
    tar_flipped = np.flip(tar, axis = flip_dims_target)

    return inp_flipped, tar_flipped

def convert_to_labels(inp: np.ndarray):
    print("INP shape:", inp.shape)
    inp[:,:, 2] = 1

    out_image = np.argmax(inp,axis = 2)
    # .reshape(1, inp.shape[0], inp.shape[1])
...
class Compose:
    """
    Base class - compose several transforms together
    """
